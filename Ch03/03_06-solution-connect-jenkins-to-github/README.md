...
- Create a new pipeline project in your Jenkins server.
  ...
  - Select `GitHub Project` and paste in the repo URL.
    - _NOTE: This step is optional.  It only creates a link to the repo on the project home page._
  ...
  - `Save` &rarr; `Build Now`.
  - _NOTE: The project must run at least one successful build before connecting to GitHub.  This allows Jenkins to read the configuration from the repo._
...
- Go back to the GitHub repo and configure the settings to create a webhook for the project you just created.
  ...
  - _NOTE: Please be sure to include the trailing slash on `github-webhook/`.  The field should be in a format similar to `http://your-jenkins-server.com/github-webhook/`._
  - Under `Content type`, select `application/json`.
  - `Add webhook`
  - _NOTE: GitHub will ping the Jenkins server and indicate a successful connection with a green check-mark next to the webhook name.  If your webhook does not indicate that it connected successfully, select `Edit` and confirm your settings again.  If needed, delete the webhook and start over._
  - Select the `<>Code` tab.
  ...
  - _NOTE: If your job is not triggered, review the configuration for the Jenkins job and the GitHub repo, making any adjustments as needed.  If needed, start again with a new job in Jenkins or with a new webhook in GitHub._
